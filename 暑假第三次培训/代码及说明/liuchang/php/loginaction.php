...
if($grade == superuser){
    ...
    $sql_select = "SELECT username,password FROM superuser WHERE username = '$username' AND password = '$password'"; //执行SQL语句
    $ret = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_array($ret); //判断用户名或密码是否正确
    if(isset($_REQUEST['authcode'])){
        session_start();
        if ($username == $row['username'] && $password == $row['password']&&strtolower($_REQUEST['authcode'])==$_SESSION['authcode']) 
    { //选中“记住我”
        if ($remember == "on") 
        { //创建cookie
    ...
    { 
        //用户名,密码或验证码错误，赋值err为1
        header("Location:login.php?err=1");
    }
} else { //用户名，密码或验证码为空，赋值err为2
