...
$id=$_GET['id'];
$sql="SELECT id,writer,title,content,releasetime,istop FROM news1 WHERE id=".$id;
...
<body>
	...
	<a href="updatenews1.php">返回管理页面</a>
<center>	
<form action="update1news.php?action=superuser&id=<?php echo $id;?>" method="post">
	<table>
		        <tr>
					<td>置顶：</td>
				...
				<tr>
					<td>作者：</td>
					<td><input type="text" name="writer" required="required" value="<?php echo $row['writer']?>" id="title"></td>
				</tr>
		        <tr>
					<td>标题：</td>
				...
				<tr>
					<td>内容：</td>
					<td>
						<input type="content" name="content" required="required" id="content" value="<?php echo $row['content']?>">
					</td>
				</tr>
				<tr>
					<td>发布时间：</td>
