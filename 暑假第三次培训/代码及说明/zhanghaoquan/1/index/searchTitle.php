<?php
    ...
    $link = connectDb();
    if($title !=''){
        $result = mysqli_query($link, "SELECT * FROM main where title like '%$title%'");
                ...
                <div class="rightmain">
                    ...
                    <?php 
                        for ($i = 0; $i < $dataCount; $i++) {
                            ...
                            $result_arr['title'] = preg_replace("/[$title]/u","<font color='red'>$title</font>",$result_arr['title']);
                            $result_arr['title'] = preg_replace("/[$title]/u","",$result_arr['title'],2);
                            
                            echo "<li><a href='zw01.php' target='_blank' title='中国共产党统一战线工作条例(示例)'>{$result_arr['title']}</a></li>";
                        }
                    ?>
                    </div>
                    <div class="rightdate">
                    <?php for ($i = 0; $i < $dataCount; $i++) {
                            echo "<li>{$result_arr['time']}</li>";
                        }
                    ?>
                    </div>
                    <div class="back">
                        <a href="llxx.php"><input type="button" value="返回"></a>
                    </div>
                </div>
            </div>
        </div>
