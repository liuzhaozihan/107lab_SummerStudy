			...
			<?php
				...
				if(!empty($_SESSION['myname'])){
					...
					echo "</br>";
     				...
     				echo "&emsp;|&emsp;<a  href='php/user_manage.php?myname=$myname'>账号管理</a>";
     				echo "&emsp;|&emsp;<a href='php/login.php?exit=1'>退出</a>";
     				echo "&emsp;|&emsp;<a  href='php/Publish_News.php'>新闻发布</a>";
				}else{
				    echo "</br>";
					echo  "&emsp;<a  href=\"php/login.php\">登录</a>&emsp;|&emsp;<a  href=\"php/register.php\">注册</a>";
				}
				echo "</p>";
				
			?>
			<form class="header_search" action="php/Search.php" method="post">
				<div class="mask1"></div>
                    <input class="search" placeholder="站内搜索" type="text" name="keywords">
        		...
        		<div class="tab3">
        			...
        			<ul class="tab3_2">
        				<?php
					        ...
					        $sql="select * from Theory order by istop desc,createtime desc";
						    $res=$link->query($sql);
						    $num=1;
						    while($num<8&$row=mysqli_fetch_assoc($res)){
						    	    $time=$row['createtime'];
  									$nowtime = date('m-d', strtotime($time)); 
						    	    echo "<li class=\"{$row['istop']}\"><a href=\"\" class=\"other_1\">&emsp;<img src=\"img/links_title.gif\" class=\"img2\"/>&emsp;{$row['content']}</a>
						    	    	  <span>$nowtime</span>
						    	    	  <a href=\"php/set_top.php?table=Theory&id={$row['id']}\" class=\"set_top\" style=\"width:30px;height:20px;display:inline-block; cursor: pointer;\">置顶</a>
						    	    </li>";
						    	    $num++;
						    	echo" <script  type=\"text/javascript\">
						    	   	   $(\".other_1\").click(function(){
							              	window.open(\"./html/decoration_1.html?Theory_id={$row['id']}\"); 
							            $(\".1\").children(\".set_top\").css({
        		...
        		<div class="new1">
        			...
        			<ul class="new1_2">
        				<?php
        					$link =@new mysqli('localhost','root','','HRM');
					        $link->set_charset('utf8');
					        $sql="select * from News order by istop desc,createtime desc";
						    $res=$link->query($sql);
						    $num=1;
						    while($num<8&$row=mysqli_fetch_assoc($res)){
						    	    $time=$row['createtime'];
  									$nowtime = date('m-d', strtotime($time)); 
						    	    echo "<li  class=\"{$row['istop']}\"><a href=\"\" class=\"other_1\">&emsp;<img src=\"img/jiantou.png\" />&emsp;{$row['content']}</a>
						    	    	  <span>$nowtime</span>
						    	    	   <a href=\"php/set_top.php?table=News&id={$row['id']}\" class=\"set_top\" style=\"width:30px;height:20px ;display:inline-block;cursor: pointer;\">置顶</a>
						    	    </li>";
						    	    $num++;
						    	    echo" <script  type=\"text/javascript\">
						    	   	   $(\".other_1\").click(function(){
							              	window.open(\"./html/decoration_1.html?News_id={$row['id']}\");  
							              });
							               $(\".set_top\").css({
							                		\"background-color\":\"#d0d0d0\",
							                	    \"border-radius\":\"5px\"
