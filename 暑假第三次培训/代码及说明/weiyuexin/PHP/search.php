	...
	<div class="content">
		...
		<div class="div4">
			<div class="div4_header">
				...
				<h>当前位置： &nbsp    <span>首页</span> &gt  <span>理论学习</span>  &gt </h>
			</div>
				...
				<ul>
					...
					<table   width="1000">
			 		 <?php
							...
							//执行信息查询
							$sql1 = "SELECT * FROM theory WHERE title LIKE '%$keyword%'";
							$result1 = mysqli_query($conn,$sql1);
							if (!$result1) {
                                printf("Error: %s\n", mysqli_error($conn));
                                exit();
                            }
                            $sql2 = "SELECT * FROM theory WHERE main LIKE '%$keyword%'";
							$result2 = mysqli_query($conn,$sql2);
							if (!$result2) {
                                printf("Error: %s\n", mysqli_error($conn));
                                exit();
                            }
                            $sql3 = "SELECT * FROM theory WHERE writer LIKE '%$keyword%'";
							$result3 = mysqli_query($conn,$sql3);
							if (!$result3) {
                                printf("Error: %s\n", mysqli_error($conn));
                                exit();
                            }
                            $sql4 = "SELECT * FROM news WHERE title LIKE '%$keyword%'";
							$result4 = mysqli_query($conn,$sql4);
							if (!$result4) {
                                printf("Error: %s\n", mysqli_error($conn));
                                exit();
                            }
                            $sql5 = "SELECT * FROM news WHERE main LIKE '%$keyword%'";
							$result5 = mysqli_query($conn,$sql5);
							if (!$result5) {
                                printf("Error: %s\n", mysqli_error($conn));
                                exit();
                            }
                            $sql6 = "SELECT * FROM news WHERE writer LIKE '%$keyword%'";
							...
							while($row1 = mysqli_fetch_assoc($result1)){
								...
								echo "<td align='left' height='35px'>
									...
									<a href='../HTML/showtheory.php?id={$row1['id']}'>
									{$row1['title']}</a>
									</td>";	
							...
							while($row2 = mysqli_fetch_assoc($result2)){
								...
								echo "<td align='left' height='35px'>
									<img src='../images/jiantou.png'>
									<a href='../HTML/showtheory.php?id={$row2['id']}'>
									{$row2['title']}</a>
									</td>";	
							...
							while($row3 = mysqli_fetch_assoc($result3)){
								...
								echo "<td align='left' height='35px'>
									<img src='../images/jiantou.png'>
									<a href='../HTML/showtheory.php?id={$row3['id']}'>
									{$row3['title']}</a>
									</td>";	
							...
							while($row4 = mysqli_fetch_assoc($result4)){
								$row4['title'] = str_replace($keyword,'<font color="red">'.$keyword.'</font>',$row4['title']);
								echo "<tr>";
								echo "<td align='left' height='35px'>
									<img src='../images/jiantou.png'>
									<a href='../HTML/shownews.php?id={$row4['id']}'>
									{$row4['title']}</a>
									</td>";	
								echo "<td>".date("Y-m-d H:i:s",$row4['addtime'])."</td>";
								echo "<td>  </td>";
								echo "</tr>";
							}
							while($row5 = mysqli_fetch_assoc($result5)){
								$row5['main'] = str_replace($keyword,'<font color="red">'.$keyword.'</font>',$row5['title']);
								echo "<tr>";
								echo "<td align='left' height='35px'>
									<img src='../images/jiantou.png'>
									<a href='../HTML/shownews.php?id={$row5['id']}'>
									{$row5['title']}</a>
									</td>";	
								echo "<td>".date("Y-m-d H:i:s",$row5['addtime'])."</td>";
								echo "<td>  </td>";
								echo "</tr>";
							}
							while($row6 = mysqli_fetch_assoc($result6)){
								$row6['writer'] = str_replace($keyword,'<font color="red">'.$keyword.'</font>',$row6['title']);
								echo "<tr>";
								echo "<td align='left' height='35px'>
									<img src='../images/jiantou.png'>
									<a href='../HTML/shownews.php?id={$row6['id']}'>
									{$row6['title']}</a>
									</td>";	
								echo "<td>".date("Y-m-d H:i:s",$row6['addtime'])."</td>";
								echo "<td>  </td>";
								echo "</tr>";
							}
							//释放结果集，关闭数据库
