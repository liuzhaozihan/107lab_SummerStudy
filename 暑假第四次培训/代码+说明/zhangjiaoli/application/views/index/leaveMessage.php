...
<head>
    ...
    <link href="<?php echo base_url().'style/index/' ?>css/leaveMess.css" rel="stylesheet"/> 
</head>
<body>
    <!--顶部-->
    <div class="top">
    ...
    <div class="leaveMess">
        <div class="current_site">
            ...
            <a href="<?php echo site_url().'/index/home/leaveMess'?>">我要留言</a>
        </div>
        <p>编辑留言信息：</p>
        <form class="email" method="post" action="#">
            ...
            <br/><br/>
            <span>请输入留言内容：</span>
            <textarea row="25" cols="70" name="messContent"></textarea>
        ...
        <?php
            ...
            if(preg_match($checkmail,$mail) && !empty($_POST['messContent'])){   
            //用正则表达式函数进行判断
               $content=$_POST['messContent'];
               $sql="INSERT mes_board(mail, content, date) VALUES('{$mail}','{$content}','{$date}');";
               $res=$mysqli->query($sql);
               if($res){
                    $url=site_url().'/index/home/leaveMess';
                    $mes="留言成功！";
            ...
            }else if(!preg_match($checkmail,$mail)){
               $url=site_url().'/index/home/leaveMess';
               $mes="电子邮箱格式不正确！留言失败！";
               echo "<script type='text/javascript'>alert('{$mes}');
               location.href='{$url}';
               </script>";
            }else if(empty($POST['messContent'])){
               $url=site_url().'/index/home/leaveMess';
               $mes="留言不能为空！留言失败！";
               echo "<script type='text/javascript'>alert('{$mes}');
               location.href='{$url}';
